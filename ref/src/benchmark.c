...
{
  printf("Benchmarking MEDS. Rounds: %i. Warmup: %i\n", ROUNDS, WARMUP);
  ...
  if (mode == 0 || mode == 3)
  {
    ...
    printf("  Warmup\n");
    for (int i = 0; i < WARMUP; i++)
      crypto_sign_open((unsigned char *)msg, (unsigned long long *)&sig_len, sig, sig_len, pk);
  ...
  if (mode == 0 || mode == 1)
  {
    float cycles = median_2(keygen_cycles, ROUNDS + 1, 0);
    printf("> Keygen:\n");
    printf("  Median cycles: %f\n", cycles);
    printf("  Time:          %f\n", cycles / osfreq());
  }

  if (mode == 0 || mode == 2)
  {
    float cycles = median_2(sign_cycles, ROUNDS + 1, 0);
    printf("> Sign:\n");
    printf("  Median cycles: %f\n", cycles);
    printf("  Time:          %f\n", cycles / osfreq());
  }
  ...
  if (mode == 0 || mode == 3)
  {
    float cycles = median_2(verify_cycles, ROUNDS + 1, 0);
    printf("> Verify:\n");
    printf("  Median cycles: %f\n", cycles);
    printf("  Time:          %f\n", cycles / osfreq());
  }

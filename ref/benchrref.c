...
int test_pmod_mat_syst_nct(pmod_mat_t *M, int M_r, int M_c)
{
  int ret = 0;
  ...
  for (int r = 0; r < M_r; r++)
  {
    ...
    if (Mrr == 0)
    {
      for (int r2 = r + 1; r2 < M_r; r2++)
      {
        // If there is no point in adding the row, continue
        if (pmod_mat_entry(M, M_r, M_c, r2, r) == 0)
          continue;

        // Otherwise, add the row and break
        for (int c = r; c < M_c; c++)
        {
          uint64_t val = pmod_mat_entry(M, M_r, M_c, r2, c);
          uint64_t Mrc = pmod_mat_entry(M, M_r, M_c, r, c);
          pmod_mat_set_entry(M, M_r, M_c, r, c, (Mrc + val) % MEDS_p);
        }
        break;
      }
    }
  ...
  for (int r = 0; r < M_r; r++)
  {
    ...
    for (int r2 = r + 1; r2 < M_r; r2++)
    {
      ...
      for (int c = r; c < M_c; c += 4)
      {
        ...
        // Reduce to a value between 0 and MEDS_p - 1: (constant time?)
        // uint16x4_t cmp_res = vcge_u16(val_16x4, MEDS_p_16x4);
        // uint16x4_t diff = vsub_u16(val_16x4, MEDS_p_16x4);
        // val_16x4 = vbsl_u16(cmp_res, diff, val_16x4);
...
int main(int argc, char *argv[])
{
  ...
  for (int round = 0; round < rref_ROUNDS; round++)
  {
    ...
    pmod_mat_set_entry(A1, A_ROWS, A_COLS, 4, 4, 0);
    pmod_mat_set_entry(A2, A_ROWS, A_COLS, 4, 4, 0);
    ...
    test_pmod_mat_syst_nct(A2, A_ROWS, A_COLS);

                                       ...
                                       size_t inlen)
{
  ...
  while (pos + inlen >= r)
  {
    for (i = pos; i < r; i++)
    {
      s[5 * (i / 8)] ^= (uint64_t)*in1++ << 8 * (i % 8);
      s[1 + 5 * (i / 8)] ^= (uint64_t)*in2++ << 8 * (i % 8);
      s[2 + 5 * (i / 8)] ^= (uint64_t)*in3++ << 8 * (i % 8);
      s[3 + 5 * (i / 8)] ^= (uint64_t)*in4++ << 8 * (i % 8);
      s[4 + 5 * (i / 8)] ^= (uint64_t)*in5++ << 8 * (i % 8);
    }
    inlen -= r - pos;
...
{
  s[5 * (pos / 8)] ^= (uint64_t)p << 8 * (pos % 8);
  s[5 * (r / 8 - 1)] ^= 1ULL << 63;
  s[1 + 5 * (pos / 8)] ^= (uint64_t)p << 8 * (pos % 8);
  s[1 + 5 * (r / 8 - 1)] ^= 1ULL << 63;
  s[2 + 5 * (pos / 8)] ^= (uint64_t)p << 8 * (pos % 8);
  s[2 + 5 * (r / 8 - 1)] ^= 1ULL << 63;
  s[3 + 5 * (pos / 8)] ^= (uint64_t)p << 8 * (pos % 8);
  s[3 + 5 * (r / 8 - 1)] ^= 1ULL << 63;
  s[4 + 5 * (pos / 8)] ^= (uint64_t)p << 8 * (pos % 8);
  s[4 + 5 * (r / 8 - 1)] ^= 1ULL << 63;
}
...
static unsigned int keccak_squeeze_vec5(uint8_t *out1, uint8_t *out2, uint8_t *out3, uint8_t *out4, uint8_t *out5, size_t outlen, uint64_t s[125], unsigned int pos, unsigned int r)
{
  ...
  while (outlen)
  {
    ...
    {
      *out1++ = s[5 * (i / 8)] >> 8 * (i % 8);
      *out2++ = s[1 + 5 * (i / 8)] >> 8 * (i % 8);
      *out3++ = s[2 + 5 * (i / 8)] >> 8 * (i % 8);
      *out4++ = s[3 + 5 * (i / 8)] >> 8 * (i % 8);
      *out5++ = s[4 + 5 * (i / 8)] >> 8 * (i % 8);
    }
    outlen -= i - pos;
    pos = i;
  }
...
{
  PROFILER_START("shake256_absorb_once");
  keccak_absorb_once(state->s, SHAKE256_RATE, in, inlen, 0x1F);
  state->pos = SHAKE256_RATE;
  PROFILER_STOP("shake256_absorb_once");
}
...
{
  PROFILER_START("shake256_squeezeblocks");
  keccak_squeezeblocks(out, nblocks, state->s, SHAKE256_RATE);
  PROFILER_STOP("shake256_squeezeblocks");
}

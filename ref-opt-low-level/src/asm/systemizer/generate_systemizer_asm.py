...
def add_elimination_row_eliminate_loop2_scalar(context, asm):
    # Load Mrc = M[r][c] and Mr2c = M[r2][c]
    add(asm, 1, f"ldrh {R_Mrc_h}, [{R_Mrc_l}], #2") # + #2 is done at the store in the end of this loop
    add(asm, 1, f"ldrh {R_Mr2c_h}, [{R_Mr2c_l}]")
    # Compute T1 = Mrc * Mr2r
...
def add_elimination_row_eliminate_loop1(context, asm):
    ## INITIALIZE VARIABLES
    # Load R_Mcr2_i = M[r2][r] = M + 2 * M_c * r2 + 2 * r
    add(asm, 1, f"madd {R_Mr2c_l}, {R_Mc}, {R_loop2}, {R_loop1}")
    add(asm, 1, f"add {R_Mr2c_l}, {R_M}, {R_Mr2c_l}, lsl #1")
    ...
    # add_elimination_row_eliminate_loop2_neon_16x4(context, asm)
    # add(asm, 1, f"add {R_loop3}, {R_loop3}, #4")
    ...
    add_elimination_row_eliminate_loop2_scalar(context, asm)
    add(asm, 1, f"add {R_loop3}, {R_loop3}, #1")
...
def add_elimination_loop(context, asm):
    ...
    add(asm, 1, f"add {R_Mrc_l}, {R_M}, {R_Mrr_i}, lsl #1")
    add_loop(asm, "elimination_eliminate_rows_loop", R_loop2, R_loop1, context.Mr, 1, lambda: add_elimination_row_eliminate_loop1(context, asm))
...
def generate_systemizer_asm(context, fun_id):
    ...
    if context.do_backsub:
        add_loop(asm, "backsub_loop", R_loop1, f"#{context.max_r}", "#0", -1, lambda: add_backsub_loop(context, asm))
...
if __name__ == "__main__":
    generate_systemizer_file("pmod_mat_syst_test", Context(4, 4, 4, False, False, 4093, 12))

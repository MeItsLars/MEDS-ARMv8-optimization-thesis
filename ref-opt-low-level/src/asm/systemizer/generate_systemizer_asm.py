...
def add_reduce(asm, rn_src, rn_tmp, rn_dst, MEDS_p, GFq_bits):
...
def add_reduce_neon(asm, rn_src, rn_tmp, rn_dst, GFq_bits):
    # Apply two reductions
...
def add_freeze_neon(asm, rn_src, rn_tmp, rn_dst):
...
def add_freeze_reduce_neon(asm, rn_src, rn_tmp, rn_tmph, rn_dst, GFq_bits):
    add_reduce_neon(asm, rn_src, rn_tmp, rn_dst, GFq_bits)
    add_freeze_neon(asm, rn_dst, rn_tmph, rn_dst)

def add_freeze(asm, rn_src, rn_tmp, rn_dst, MEDS_p):
    # Remove one final MEDS_p if the value in the lane is at least MEDS_p'
    add(asm, 1, f"cmp {rn_src}, #{MEDS_p}")
    add(asm, 1, f"csel {rn_tmp}, {R_MEDSp}, xzr, ge")
    add(asm, 1, f"sub {rn_dst}, {rn_src}, {rn_tmp}")

def add_freeze_reduce(asm, rn_src, rn_tmp, rn_dst, MEDS_p, GFq_bits):
    add_reduce(asm, rn_src, rn_tmp, rn_dst, MEDS_p, GFq_bits)
    add_freeze(asm, rn_dst, rn_tmp, rn_dst, MEDS_p)

def add_swap(context, asm):
    return

def add_elimination_row_zero_fix_loop2_neon(context, asm):
    # Load Mrc = M[r][c] and Mr2c = M[r2][c]
    add(asm, 1, f"ld1 {{{RN_Mrc}}}, [{R_Mrc_i}]") # + #2 is done at the store in the end of this loop
    add(asm, 1, f"ld1 {{{RN_Mr2c}}}, [{R_Mr2c_i}], #8")
    ...
    # Reduce RN_T2w
    # add_freeze_neon(asm, RN_T2, RN_T1, RN_T2)
    # Store the result into M[r][c]
    add(asm, 1, f"st1 {{{RN_Mr2c}}}, [{R_Mrc_i}], #8")

def add_elimination_row_zero_fix_loop2(context, asm):
    # Load Mrc = M[r][c] and Mr2c = M[r2][c]
    add(asm, 1, f"ldrh {R_Mrc_h}, [{R_Mrc_i}]") # + #2 is done at the store in the end of this loop
    add(asm, 1, f"ldrh {R_Mr2c_h}, [{R_Mr2c_i}], #2")
    # Compute RN_T1 = Mr2c if Mrr is zero, else zero (we can't use csel)
    ...
    # Store the result into M[r][c]
    add(asm, 1, f"strh {R_T1_h}, [{R_Mrc_i}], #2")
...
def add_elimination_row_zero_fix_loop1(context, asm):
    ...
    add(asm, 1, f"add {R_Mr2c_i}, {R_M}, {R_Mr2c_i}, lsl #1")
    ...
    # Loop
    add_loop(asm, "elimination_row_zero_fix_inner_loop_neon", R_loop3, R_loop1, R_Mc, 0, lambda: add_elimination_row_zero_fix_loop2_neon(context, asm))
    # Loop
...
def add_normalize_row(context, asm):
    ...
    # Store the result into M[r][c]
    add(asm, 1, f"strh {R_Mrc_h}, [{R_Mrc_i}], #2")
...
def add_elimination_loop(context, asm):
    ...
    add(asm, 1, f"add {R_Mrc_i}, {R_M}, {R_Mrc_i}, lsl #1")

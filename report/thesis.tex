...
We are able to replace the sums with closed-form expressions because of known arithmetic series. In the final step, as $n \geq m$ and therefore $m^2n \geq m^3$, we find that the complexity of the basic algorithm is $\mathcal{O}(m^2n)$.

We consider the changes that the three optional features of the algorithm bring to the complexity:
\begin{enumerate}
  \item As $r_\text{max}$ is only ever $m$ or $m-1$, we can see that the complexity of the algorithm does not change based on the value of $r_\text{max}$.
  ...
  \end{align*}
  Again, following a similar approach as above, we find that this results in a complexity of $\mathcal{O}(m^2 + nm^2 - m^2) = \mathcal{O}(nm^2)$, which does not change the complexity of the algorithm.
\end{enumerate}
...
For the basic algorithm, the number of instructions required can be calculated as follows:
\begin{itemize}
  \item $\sum_{r=0}^{r\_\text{max}} \sum_{r_2=r+1}^{m} \sum_{c=r}^{n} \frac{1}{8}$ add and freeze instructions
  \item $\sum_{r=0}^{r\_\text{max}} 1$ finite field inversions
  \item $\sum_{r=0}^{r\_\text{max}} \sum_{c=r}^{n} (\frac{1}{8} \cdot 2)$ multiply and reduce instructions
  \item $\sum_{r=0}^{r\_\text{max}} \sum_{c=r}^{n} \frac{1}{8}$ combinations of reduced results
  \item $\sum_{r=0}^{r\_\text{max}} \sum_{c=r}^{n} (\frac{1}{8} \cdot 2)$ multiply and reduce instructions
  \item $\sum_{r=0}^{r\_\text{max}} \sum_{r_2=r+1}^{m} \sum_{c=r}^{n} (\frac{1}{8} \cdot 2)$ multiply and reduce instructions
  \item $\sum_{r=0}^{r\_\text{max}} \sum_{r_2=r+1}^{m} \sum_{c=r}^{n} \frac{1}{8}$ combinations of reduced results
  \item $\sum_{r=0}^{r\_\text{max}} \sum_{r_2=r+1}^{m} \sum_{c=r}^{n} \frac{1}{8}$ add, subtract, and freeze instructions
\end{itemize}
Combining these results (using the same technique as for the complexity analysis), we obtain $1$.
...
\midrule
    Matrix Syst. & $A$: $k \times k$& & 105571 & 100 & 1055.71\\
    Matrix Systemization & $A$: $k \times 2k$& & 209888 & 100 & 2098.88\\
    Matrix Syst. (bsub$^{*}$) & $A$: $n \times 2n$& & 329936 & 100 & 3299.36\\
    Matrix Syst. (bsub$^{*}$) & $A$: $m \times 2m$& & 295878 & 100 & 2958.78\\
    Matrix Syst. (bsub$^{*}$) & $A$: $k \times 2k$& & 295632 & 100 & 2956.32\\
    Matrix Syst. (n-1$^{**}$) (bsub$^{*}$) & $A$: $n \times 2m$& & 306502 & 100 & 3065.02\\
    Matrix Syst. (swap$^{***}$) (bsub$^{*}$) & $A$: $m-1 \times m$& & 133584 & 100 & 1335.84\\
\midrule

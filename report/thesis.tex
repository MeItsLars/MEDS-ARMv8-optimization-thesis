...
\todo[inline]{This is the only remaining section that still needs to be changed.}
...
% A section where we compare this lower bound to a lower bound that results from the naive algorithm, and calculate a difference factor.
...
% $xy + xy \cdot \frac{1}{8}z \cdot (8 + 2 \cdot (1 + 5) + 1 + 1 + (1 + 1 + 3) + 1) = xy + \frac{28}{8}xyz$.
...
The speedup factors are larger on the Apple M2 than on the ARM Cortex-A72, especially for the variants with the optimized hash structure. This is due to the fact that the Apple M2 is able to utilize the cryptographic extension of the ARMv8 architecture, which contains instructions that accelerate the parallel performance of the Keccak-f[1600] permutation.

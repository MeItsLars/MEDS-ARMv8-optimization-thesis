...
\lstdefinestyle{CStyle}{
  ...
  showtabs=false,                  
  tabsize=2,
  morekeywords={uint64_t, pmod_mat_entry, uint16x4_t, uint32x4_t, uint64x2_t, vmull_u32, vget_low_u32, vmull_high_u32, vuzp2q_u32, vshrq_n_u32, vmlsq_u32, vmovn_u32}
}
...
The high-level optimization approach focuses on optimizing the MEDS implementation by parallelizing over the challenge space. MEDS uses a large number ($t$) of challenges in both signing and verification, which are all computed independently. Additionally, the computation for each challenge is exactly the same for signing and very similar for verification. This means that we can parallelize the computation of the challenges over the challenge space.

Before we can parallelize the computation of the challenges, we need to determine the number of challenges that we will compute in parallel. The challenge computation is executed over $\mathbb{F}_{4093}$, which means that the field elements can be stored in 12 bits (which fits in a 16-bit register). This means that we can store up to 8 field elements in a single 128-bit NEON register. From this, there are two possible parallelization factors we can consider.
\begin{enumerate}
  \item \textbf{4-Way Parallelization}:\\
  Nearly all operations on field elements allow the values to stay in 16-bit registers. An exception is multiplication: the result of a multiplication of two field elements is a 24-bit value, which fits in a 32-bit register. This means that we can compute 4 multiplications at the same time using 128-bit NEON registers. Because of this limitation, we can choose to compute 4 challenges at a time.
  \item \textbf{8-Way Parallelization}:\\
  We can work around the multiplication limitation by using instructions such as \texttt{umull} and \texttt{umull2} to multiply the lower and upper 4 values (respectively) of two 128-bit NEON registers, after which we can reduce the results and combine them back into a single 128-bit register. Using this technique, we can compute 8 challenges at the same time.
\end{enumerate}
...
In this section, we will discuss the high-level optimization of the MEDS implementation. We will first discuss the alterations that need to be made to the underlying datatypes to allow for high-level parallelization in Section \ref{sec:highleveldatatypes}. We will then discuss how we can use these datatypes to parallelize the supplemental algorithms in Section \ref{sec:suppalgorithmsparallelization}. Next, we will discuss the actual parallelization of the challenge computations in Section \ref{sec:challengeparallelization}. Finally, we will discuss the limitations of the high-level optimization approach in Section \ref{sec:highlevellimitations}.

% In the next subsection, we will explain our alteration of the datatypes such that parallelization becomes possible
\subsection{Datatypes and Functions}
\label{sec:highleveldatatypes}

\subsection{Supplemental Algorithms}
\label{sec:suppalgorithmsparallelization}
In order to parallelize the challenge computations, we need to modify the supplemental algorithms that are used in the MEDS implementation. In the reference implementation, these algorithms perform operations for matrices or values that belong to a single challenge. We need to modify these algorithms such that they can perform operations for multiple challenges at the same time. The following algorithms need to be modified:
...
\subsection{Challenge Parallelization}
\label{sec:challengeparallelization}
...
ref uses long algorithm\todo{Git}

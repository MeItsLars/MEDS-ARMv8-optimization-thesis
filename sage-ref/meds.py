...
# This file was *autogenerated* from the file meds.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_1 = Integer(1); _sage_const_2 = Integer(2); _sage_const_8 = Integer(8); _sage_const_4 = Integer(4); _sage_const_48 = Integer(48); _sage_const_256 = Integer(256)#!/usr/bin/env sage
...
class MEDSbase:
  ...
  def crypto_sign_keypair(self):
    ...
    G[_sage_const_0 ] = ExpandSystMat(sigma_G0, GFq, k, m, n)
    ...
    for i in range(_sage_const_1 , s):
      while True: # repeat until A[i] and B[i] are invertible and G[i] has systematic form
        ...
        G0prime = Ti * G[_sage_const_0 ]

        logging.debug(f"G0prime:\n%s", G0prime)


        check_A_i, check_B_i = solve([matrix(GFq, m, n, G0prime.rows()[j]) for j in range(_sage_const_2 )])
        ...
        G[i] = pi(A_i, B_i, G[_sage_const_0 ])

        logging.debug(f"G[{i}]:\n%s", G[i])

        T_inv[i] = G[i].submatrix(_sage_const_0 ,_sage_const_0 ,k,k)
    ...
    for A_inv_i in A_inv[_sage_const_1 :]:
      sk += Compress(A_inv_i)

    for B_inv_i in B_inv[_sage_const_1 :]:
      sk += Compress(B_inv_i)

    for T_inv_i in T_inv[_sage_const_1 :]:
      sk += Compress(T_inv_i)
  ...
  def crypto_sign(self, sk, msg):
    ...
    GF_BITS = ceil(log(q, _sage_const_2 ))
    ...
    l_Fq_nn = ceil((GF_BITS * n * n) / _sage_const_8 )
    l_Fq_mm = ceil((GF_BITS * m * m) / _sage_const_8 )
    l_Fq_kk = ceil((GF_BITS * k * k) / _sage_const_8 )
    ...
    for i in range(t):
      while True:
        ...
        seed_buf = alpha + sigma[i] + i.to_bytes(_sage_const_4 , "little")
        ...
        M_tilde[i] = matrix(GFq, _sage_const_2 , k, ExpandFqs(shake, _sage_const_2 *k, GFq))
  ...
  def crypto_sign_open(self, pk, ms):
    ...
    GFq = GF(q)
    GF_BYTES = ceil(log(q, _sage_const_2 ) / _sage_const_8 )
    GF_BITS = ceil(log(q, _sage_const_2 ))
    ...
    l_Gi = ceil((k-_sage_const_2 )*(m*n-k) * GF_BITS / _sage_const_8 )
    ...
    kappa = ms[:ceil(_sage_const_2 *k * GF_BITS / _sage_const_8 )*w]
    ms = ms[ceil(_sage_const_2 *k * GF_BITS / _sage_const_8 )*w:]
    ...
    l_Fq_kk = ceil((GF_BITS * _sage_const_2  * k) / _sage_const_8 )
    ...
    for i in range(t):
      if h[i] > _sage_const_0 :
        ...
        kappa_i = Decompress(kappa[f_ms : f_ms + l_Fq_kk], GFq, _sage_const_2 , k)
        f_ms += l_Fq_kk
...
#              break
          ...
          M_hat_i = matrix(GFq, _sage_const_2 , k, ExpandFqs(shake, _sage_const_2 *k, GFq))

          logging.debug(f"M_hat[{i}]:\n%s", M_hat_i)
  
          C_hat = M_hat_i * G[_sage_const_0 ]
...
#          logging.debug(f"G0_prime[{i}]:\n%s", G0_prime)
          ...
          G_hat[i] = pi(A_hat_i, B_hat_i, G[_sage_const_0 ])
...
if __name__ == "__main__":
  ...
  parser.add_argument("-s", "--seed", default=_sage_const_0 , help = "initial entropy seed for deterministic testing")
  ...
  try:
    ...
    entropy_input = int(args.seed).to_bytes(_sage_const_48 , 'little')
    randombytes_init(bytes(entropy_input), None, _sage_const_256 )

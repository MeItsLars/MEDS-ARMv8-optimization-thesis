...
def add_loop_unroll(loop_from, loop_to, direction, loop_code_fun):
    loop_var = loop_from
    if direction == 1:
        loop_var += 1
    elif direction == -1:
        loop_var -= 1
    while loop_var != loop_to:
        loop_code_fun(loop_var)
        if direction >= 0:
            loop_var += 1
        else:
            loop_var -= 1
...
def add_backsub_column_loop_neon_16x4(context, asm, r, r2, c):
    # Load Mrc and Mr2c
    if 2 * context.Mc * r + 2 * c < 256:
        add(asm, 1, f"ldr {RN_Mrc_d}, [{R_M}, #{2 * context.Mc * r + 2 * c}]")
    else:
        add(asm, 1, f"mov {R_T1}, #{2 * context.Mc * r + 2 * c}")
        add(asm, 1, f"ldr {RN_Mrc_d}, [{R_M}, {R_T1}]")
    if 2 * context.Mc * r2 + 2 * c < 256:
        add(asm, 1, f"ldr {RN_Mr2c_d}, [{R_M}, #{2 * context.Mc * r2 + 2 * c}]")
    else:
        add(asm, 1, f"mov {R_T1}, {2 * context.Mc * r2 + 2 * c}")
        add(asm, 1, f"ldr {RN_Mr2c_d}, [{R_M}, {R_T1}]")
    # Compute T1 = Mrc * Mr2r
    add(asm, 1, f"umull {RN_T1_w}, {RN_Mrc}, {RN_Mr2r}")
    # Reduce T1
    add_freeze_reduce_neon_16x4(asm, RN_T1_w, RN_T2_w, RN_T2, RN_T1, context.GFq_bits)
    ...
    if 2 * context.Mc * r2 + 2 * c < 256:
        add(asm, 1, f"str {RN_Mr2c_d}, [{R_M}, #{2 * context.Mc * r2 + 2 * c}]")
    else:
        add(asm, 1, f"mov {R_T1}, {2 * context.Mc * r2 + 2 * c}")
        add(asm, 1, f"str {RN_Mr2c_d}, [{R_M}, {R_T1}]")

def add_backsub_column_loop_scalar(context, asm, r, r2, c):
    # Load Mrc and Mr2c
    add(asm, 1, f"ldrh {R_Mrc_h}, [{R_M}, #{2 * context.Mc * r + 2 * c}]")
    add(asm, 1, f"ldrh {R_Mr2c_h}, [{R_M}, #{2 * context.Mc * r2 + 2 * c}]")
    # Compute T1 = Mrc * Mr2r
    add(asm, 1, f"mul {R_T1}, {R_Mrc}, {R_Mr2r}")
    # Reduce T1
    add_freeze_reduce(asm, R_T1, R_T2, R_T1, context.MEDS_p, context.GFq_bits)
    ...
    add_freeze(asm, R_T2, R_T1, R_Mr2c, context.MEDS_p)
    # Store the result into M[r2][c]
    add(asm, 1, f"strh {R_Mr2c_h}, [{R_M}, #{2 * context.Mc * r2 + 2 * c}]")

def add_backsub_inner_loop(context, asm, r, r2):
    ## INITIALIZE VARIABLES
    # Load R_Mr2r = M[r2][r] = M + 2 * M_c * r2 + 2 * r
    add(asm, 1, f"ldrh {R_Mr2r_h}, [{R_M}, #{2 * context.Mc * r2 + 2 * r}]")
    # Store M[r2][r] into RN_Mr2r
    add(asm, 1, f"dup {RN_Mr2r}, {R_Mr2r_h}")

    # Load R_Mrc = M[r][r] = M + 2 * M_c * r + 2 * r
    add(asm, 1, f"ldrh {R_Mrc_h}, [{R_M}, #{2 * context.Mc * r + 2 * r}]")
    # Load R_Mr2c = M[r2][r] = M + 2 * M_c * r2 + 2 * r
    add(asm, 1, f"ldrh {R_Mr2c_h}, [{R_M}, #{2 * context.Mc * r2 + 2 * r}]")
    # Compute T1 = Mrc * Mr2r
    add(asm, 1, f"mul {R_T1}, {R_Mrc}, {R_Mr2r}")
    # Reduce T1
    add_freeze_reduce(asm, R_T1, R_T2, R_T1, context.MEDS_p, context.GFq_bits)
    ...
    add(asm, 1, f"strh {R_Mr2c_h}, [{R_M}, #{2 * context.Mc * r2 + 2 * r}]")

    c = context.max_r
    while c <= context.Mc - 4:
        add_backsub_column_loop_neon_16x4(context, asm, r, r2, c)
        c += 4
    while c < context.Mc:
        add_backsub_column_loop_scalar(context, asm, r, r2, c)
        c += 1

def add_backsub_outer_loop(context, asm, r):
    add_loop_unroll(0, r, 0, lambda r2: add_backsub_inner_loop(context, asm, r, r2))
...
def generate_systemizer_asm(context, fun_id):
    ...
    if context.do_backsub:
        add_loop_unroll(context.max_r - 1, 0, -2, lambda r: add_backsub_outer_loop(context, asm, r))
